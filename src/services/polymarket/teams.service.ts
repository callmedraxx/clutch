...
export class TeamsService {
  ...
  async upsertTeams(teams: Team[], league: string): Promise<void> {
    if (useDatabase) {
      await this.upsertTeamsToDatabase(teams, league);
    } else {
      this.upsertTeamsToMemory(teams, league);
    }
  }
  ...
  private getTeamFromMemory(id: number, league?: string): Team | null {
    if (league) {
      const leagueTeams = inMemoryTeams.get(league);
      return leagueTeams?.get(id) || null;
    }

    // Search across all leagues
    for (const leagueTeams of inMemoryTeams.values()) {
      const team = leagueTeams.get(id);
      if (team) {
        return team;
      }
    }
