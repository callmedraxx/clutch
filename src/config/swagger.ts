      ...
      schemas: {
        TransformedOutcome: {
          ...
          properties: {
            ...
            groupItemThreshold: { type: 'string', description: 'Threshold for group items' },
            isWinner: { type: 'boolean', description: 'True if this outcome won (for resolved markets)' },
            marketId: { type: 'string', description: 'Market ID this outcome belongs to (for multi-market events like live games)' },
            marketQuestion: { type: 'string', description: 'Market question/type (e.g., "Moneyline", "Spread", "Totals") for identification' },
            active: { type: 'boolean', description: 'Market active status' },
            closed: { type: 'boolean', description: 'Market closed status' },
          },
        },
        TransformedMarket: {
          type: 'object',
        ...
        TransformedEvent: {
          ...
          properties: {
            ...
            isBinaryOutcome: { type: 'boolean', description: 'Computed: true if event has exactly 2 outcomes (typically Yes/No)' },
            isMultiOutcome: { type: 'boolean', description: 'Computed: true if event has more than 2 outcomes' },
